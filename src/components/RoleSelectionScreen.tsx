    ...
    >
      ...
      <div className="w-full max-w-sm sm:max-w-md my-auto flex flex-col items-center text-center">
        ...
        <div
          ...
          className="grid grid-cols-2 gap-2.5 sm:gap-4 w-full max-w-[270px] sm:max-w-md mx-auto"
        >
          ...
          <button
            ...
            className={`w-full aspect-square bg-white rounded-xl sm:rounded-3xl p-2.5 sm:p-5 flex flex-col items-center justify-center cursor-pointer transition-all duration-200 outline-none select-none active:scale-[0.97] ${
              selectedRole === 'mother'
                ? 'ring-2 ring-[#26374a] shadow-md -translate-y-0.5 bg-slate-50/40'
                : 'border border-slate-200/90 hover:border-slate-300 hover:shadow-xs'
            }`}
          >
            {/* Circular badge */}
            <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-[#e3eef5] text-[#26374a] font-bold text-sm sm:text-lg flex items-center justify-center mb-1.5 sm:mb-2.5 shrink-0">
              أم
            </div>
            ...
            {/* Greeting */}
            <span className="text-[10px] sm:text-xs text-[#7e8f9f] font-normal leading-tight">
              أهلاً بكِ
            </span>
          </button>
        ...
        <div className="w-full max-w-[270px] sm:max-w-md mx-auto mt-4 sm:mt-6 min-h-[44px] sm:min-h-[50px] flex flex-col items-center justify-center">
          {selectedRole && !isLoggedIn && (
            <button
              id="enter-button"
              type="button"
              onClick={handleLogin}
              className="w-full py-2.5 sm:py-3.5 px-4 sm:px-5 rounded-xl sm:rounded-2xl bg-[#26374a] hover:bg-[#1d2b3a] active:bg-[#16212c] active:scale-[0.99] text-white text-xs sm:text-sm font-semibold flex items-center justify-center gap-1.5 sm:gap-2 shadow-xs transition-all duration-150 cursor-pointer animate-in fade-in zoom-in-95 duration-200"
            >
